        ...
        book.for_each_mut(|section: &mut BookItem| {
            if let BookItem::Chapter(ref mut ch) = *section {
                if let Some(ref chapter_path) = ch.path {
                    ...
                    let content = replace_all(&ch.content, base, Some(fallback), chapter_path, 0);
                    ch.content = content;
                }
            }
        });
...
fn replace_all<P1, P2>(s: &str, path: P1, fallback: Option<P1>, source: P2, depth: usize) -> String
where
    P1: AsRef<Path>,
    P2: AsRef<Path>,
{
    ...
    for link in find_links(s) {
        ...
        match link.render_with_path(&path, fallback) {
            Ok(new_content) => {
...
mod tests {
    ...
    fn test_replace_all_escaped() {
        ...
        {{#include file.rs}} << an escaped link!
        ```";
        assert_eq!(replace_all(start, "", None, "", 0), end);
    }
